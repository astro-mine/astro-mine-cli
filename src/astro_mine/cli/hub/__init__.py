...
class _Command:
    """`astro-mine hub <verb>` — publish, discover and verify artifacts."""

    name = "hub"
    help = "publish, discover and verify artifacts"

    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        _add_arguments(parser)

    def run(self, args: argparse.Namespace) -> int:
        # The backstop for the no-tracebacks-on-user-input rule. The verbs report what they
        # can name precisely; this catches the classes that are always the user's input rather
        # than a Hub bug -- an unreadable path, a manifest that does not validate, a registry
        # that answers with an error -- so a new verb cannot reintroduce a traceback by
        # forgetting a `try`. Anything else still raises: a real defect must not be dressed up
        # as bad input.
        try:
