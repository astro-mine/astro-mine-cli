...
class _Command:
    """`astro-mine bench <verb>` — fetch, score and submit benchmark runs."""

    name = "bench"
    help = "fetch, score and submit benchmark runs"

    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        _add_arguments(parser)

    def run(self, args: argparse.Namespace) -> int:
        # `eval-worker` is deliberately absent. It is the single-seed rollout argv Cloud fans
        # out (RM-P1-BENCH-11), not something a person types: it owns its own argparse, rides
        # the [cloud]/[recording] extras, and is invoked as `python -m astro_mine.bench
        # eval-worker …`. It was hidden from `astro-mine bench --help` for exactly that reason
        # and stays hidden here, reachable where its caller already looks for it.
        import sys
