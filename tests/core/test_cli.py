...
def main(argv=None):  # type: ignore[no-untyped-def]
    return _astro_mine(["core", *(argv or [])])
...
def test_validate_exit_code_and_json(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    code = main(["--json", "validate", "--kind", "objective", str(good), str(bad)])
    assert code == 1
...
def test_wheel_carries_the_cli_and_its_schemas(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheels[0]) as whl:
        ...
        assert "astro-mine-core=astro_mine.cli.core:main" in registered
    ...
    program = (
        ...
        "sys.exit(main(['validate', '--kind', 'mission', sys.argv[1]]))"
    )
