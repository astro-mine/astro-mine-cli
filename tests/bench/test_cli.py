...
Drives :func:`astro_mine.cli.bench.main` with injected stdout/stderr buffers, covering the
...
def main(argv=None, *, stdout=None, stderr=None):  # type: ignore[no-untyped-def]
    """`astro-mine bench <verb>`, with the stream injection these tests rely on.

    The platform's `bench.cli.main` took `stdout`/`stderr` kwargs so tests could capture output
    without touching the process streams. The CLI reads `sys.stdout` at call time instead, so
    the injection is done by redirecting around the call -- same observable behaviour, and the
    ...
    with contextlib.ExitStack() as stack:
        ...
        return _astro_mine(["bench", *(argv or [])])
...
def test_fetch_reports_the_store_path_and_the_signer_posture(
    ...
    monkeypatch.setattr("astro_mine.cli.bench.fetch_scenario_content", fake_fetch)
...
def test_fetch_names_the_pinned_signer_when_a_key_is_given(
    ...
    monkeypatch.setattr("astro_mine.cli.bench.fetch_scenario_content", lambda spec, **kw: ())
...
def test_score_registry_flag_is_passed_to_the_runner(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("astro_mine.cli.bench.load_runner_provider", lambda name: _Provider())
...
def test_score_does_not_swallow_a_genuine_runner_bug(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("astro_mine.cli.bench.load_runner_provider", lambda _name: _Buggy())
